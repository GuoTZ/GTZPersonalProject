...
@implementation UIDevice (GTZHardware)
...
+ (NSString *)gtz_platformString{
    NSString *platform = [self gtz_platform];
...
+ (NSUInteger)gtz_getSysInfo:(uint)typeSpecifier
{
...
+ (NSUInteger)gtz_cpuFrequency {
    return [self gtz_getSysInfo:HW_CPU_FREQ];
}

+ (NSUInteger)gtz_busFrequency {
    return [self gtz_getSysInfo:HW_BUS_FREQ];
}

+ (NSUInteger)gtz_ramSize {
    return [self gtz_getSysInfo:HW_MEMSIZE];
}

+ (NSUInteger)gtz_cpuNumber {
    return [self gtz_getSysInfo:HW_NCPU];
}


+ (NSUInteger)gtz_totalMemoryBytes
{
    return [self gtz_getSysInfo:HW_PHYSMEM];
}
