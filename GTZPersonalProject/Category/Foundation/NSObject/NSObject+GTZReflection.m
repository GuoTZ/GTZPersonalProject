...
#import "NSObject+GTZReflection.h"
#import <objc/runtime.h>
@implementation NSObject (GTZReflection)
...
{
    return [[self class] gtz_propertyKeys];
}
+ (NSArray *)gtz_propertyKeys {
...
{
    return [[self class] gtz_propertiesInfo];
}
/**
...
+ (NSArray *)gtz_propertiesInfo
{
    ...
    for (int i = 0; i < propertyCount; i++)
    {
        [propertieArray addObject:({
            ...
            NSDictionary *dictionary = [self gtz_dictionaryWithProperty:properties[i]];
...
+ (NSArray *)gtz_propertiesWithCodeFormat
{
    ...
    NSArray *properties = [[self class] gtz_propertiesInfo];
...
-(NSArray*)gtz_methodListInfo{
    ...
    for (int i = 0; i < count ; i++)
    {
        ...
        for (int index=0; index<argumentsCount; index++) {
            ...
            [arguments addObject:[[self class] gtz_decodeType:arg]];
        }
        
        NSString *returnTypeString =[[self class] gtz_decodeType:returnType];
        NSString *encodeString = [[self class] gtz_decodeType:encoding];
...
-(NSDictionary *)gtz_protocolList{
    return [[self class]gtz_protocolList];
}
+ (NSDictionary *)gtz_protocolList
{
...
+ (NSArray *)gtz_instanceVariable
{
    ...
    NSMutableArray *result = [NSMutableArray array];
    for (int i = 0; i < outCount; i++) {
        NSString *type = [[self class] gtz_decodeType:ivar_getTypeEncoding(ivars[i])];
...
#pragma mark -- help
+ (NSDictionary *)gtz_dictionaryWithProperty:(objc_property_t)property
{
...
+ (NSString *)gtz_decodeType:(const char *)cString
{
    ...
    } else
    {
        if ([[result substringToIndex:1] isEqualToString:@"^"]) {
            result = [NSString stringWithFormat:@"%@ *",
                      [NSString gtz_decodeType:[[result substringFromIndex:1] cStringUsingEncoding:NSUTF8StringEncoding]]];
        }
    }
    return result;
}
